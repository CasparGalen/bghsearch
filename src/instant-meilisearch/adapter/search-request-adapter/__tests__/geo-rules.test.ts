...
/* checking that the adaptGeoSearch function correctly handles the case where multiple
geo parameters (aroundLatLng, aroundRadius, and insideBoundingBox) are provided and
generates the correct filter string for MeiliSearch */
test('Adapt instantsearch geo parameters to meilisearch filters with aroundLatLng, radius, and insideBoundingBox', () => {
  const filter = adaptGeoSearch({
    aroundLatLng: '51.1241999, 9.662499900000057',
    aroundRadius: 1,
...
/* checking where only boundingBox is present and generate filter string for MeiliSearch */
test('Adapt instantsearch geo parameters to meilisearch filters with only a boundingBox', () => {
  const filter = adaptGeoSearch({
    insideBoundingBox: '1,2,3,4'
  })
